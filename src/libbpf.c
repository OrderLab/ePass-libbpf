...
static struct bpf_map *bpf_object__add_map(struct bpf_object *obj) {
    ...
    err = libbpf_ensure_mem((void **)&obj->maps, &obj->maps_cap, sizeof(*obj->maps),
                            obj->nr_maps + 1);
    if (err)
        return ERR_PTR(err);
...
static int bpf_object__init_internal_map(struct bpf_object *obj, enum libbpf_map_type type,
    ...
    def->max_entries = 1;
    def->map_flags =
        type == LIBBPF_MAP_RODATA || type == LIBBPF_MAP_KCONFIG ? BPF_F_RDONLY_PROG : 0;
...
static bool prog_needs_vmlinux_btf(struct bpf_program *prog) {
    if (prog->type == BPF_PROG_TYPE_STRUCT_OPS || prog->type == BPF_PROG_TYPE_LSM)
        return true;
...
static int bpf_object__collect_prog_relos(struct bpf_object *obj, Elf64_Shdr *shdr,
                                          Elf_Data *data) {
...
static int bpf_object__probe_loading(struct bpf_object *obj) {
    ...
    ret = bpf_prog_load(BPF_PROG_TYPE_SOCKET_FILTER, NULL, "GPL", insns, insn_cnt, NULL, 0, NULL, NULL);
    if (ret < 0)
        ret = bpf_prog_load(BPF_PROG_TYPE_TRACEPOINT, NULL, "GPL", insns, insn_cnt, NULL, 0, NULL, NULL);
    if (ret < 0) {
        ret = errno;
...
static int probe_kern_exp_attach_type(void) {
     ...
     */
    fd = bpf_prog_load(BPF_PROG_TYPE_CGROUP_SOCK, NULL, "GPL", insns, insn_cnt, &opts, 0, NULL, NULL);
    return probe_fd(fd);
}
...
static int probe_kern_probe_read_kernel(void) {
    ...
    fd = bpf_prog_load(BPF_PROG_TYPE_TRACEPOINT, NULL, "GPL", insns, insn_cnt, NULL, 0, NULL, NULL);
    return probe_fd(fd);
}
...
static int probe_prog_bind_map(void) {
    ...
    prog = bpf_prog_load(BPF_PROG_TYPE_SOCKET_FILTER, NULL, "GPL", insns, insn_cnt, NULL, 0, NULL, NULL);
    if (prog < 0) {
        close(map);
        return 0;
    }
...
static int probe_perf_link(void) {
    ...
    prog_fd = bpf_prog_load(BPF_PROG_TYPE_TRACEPOINT, NULL, "GPL", insns, ARRAY_SIZE(insns), NULL, 0, NULL, NULL);
    if (prog_fd < 0)
        return -errno;
...
static int probe_kern_bpf_cookie(void) {
    ...
    ret = bpf_prog_load(BPF_PROG_TYPE_KPROBE, NULL, "GPL", insns, insn_cnt, NULL, 0, NULL, NULL);
    return probe_fd(ret);
}
...
static struct kern_feature_desc {
    ...
    [FEAT_EXP_ATTACH_TYPE] =
        {
                          "BPF_PROG_LOAD expected_attach_type attribute",   probe_kern_exp_attach_type,
                          },
...
static void poison_kfunc_call(struct bpf_program *prog, int relo_idx, int insn_idx,
                              struct bpf_insn *insn, int ext_idx, const struct extern_desc *ext) {
    pr_debug("prog '%s': relo #%d: poisoning insn #%d that calls kfunc '%s'\n", prog->name,
             relo_idx, insn_idx, ext->name);
        ...
        switch (relo->type) {
            ...
            case RELO_EXTERN_CALL:
                ...
                if (ext->is_set) {
                    ...
                    poison_kfunc_call(prog, i, relo->insn_idx, insn, relo->ext_idx, ext);
                }
                break;
...
retry_load:
    ...
    ret = bpf_prog_load(prog->type, prog_name, license, insns, insns_cnt, &load_attr, enable_epass, epass_opt, epass_passopt);
    if (ret >= 0) {
...
static void fixup_log_missing_kfunc_call(struct bpf_program *prog, char *buf, size_t buf_sz,
                                         size_t log_sz, char *line1, char *line2, char *line3) {
...
static void fixup_verifier_log(struct bpf_program *prog, char *buf, size_t buf_sz) {
    ...
    for (i = 0; i < max_last_line_cnt; i++, next_line = cur_line) {
        ...
        if (str_has_pfx(cur_line, "invalid func unknown#195896080\n")) {
            ...
            fixup_log_missing_kfunc_call(prog, buf, buf_sz, log_sz, prev_line, cur_line, next_line);
            return;
        }
    }
}
...
static int kallsyms_cb(unsigned long long sym_addr, char sym_type, const char *sym_name,
                       void *ctx) {
...
static int bpf_object__resolve_ksym_func_btf_id(struct bpf_object *obj, struct extern_desc *ext) {
    ...
    if (ret <= 0) {
        pr_warn("extern (func ksym) '%s': func_proto [%d] incompatible with %s [%d]\n", ext->name,
                local_func_proto_id, mod_btf ? mod_btf->name : "vmlinux", kfunc_proto_id);
        return -EINVAL;
    }
    ...
    pr_debug("extern (func ksym) '%s': resolved to %s [%d]\n", ext->name,
             mod_btf ? mod_btf->name : "vmlinux", kfunc_id);
...
static const struct bpf_sec_def section_defs[] = {
    ...
    SEC_DEF("fmod_ret.s+", TRACING, BPF_MODIFY_RETURN, SEC_ATTACH_BTF | SEC_SLEEPABLE,
            attach_trace),
